...
async function testWithSampleFile() {
  ...
  try {
    ...
    const availableFunctions = Object.keys(module).filter(key => key.startsWith('_') && typeof module[key] === 'function');
    console.log("🔍 Available WASM functions:", availableFunctions.slice(0, 10));
    
    console.log("🎉 Basic WASM integration working!");
    
  } catch (error) {
    console.error("❌ Error:", (error as Error).message);
    console.log("💡 This is expected until we implement buffer-based file loading in the C++ bindings");
  }
}
