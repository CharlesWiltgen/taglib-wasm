...
async function demonstrateAdvancedMetadata() {
  ...
  try {
    ...
    for (const { path, format } of testFiles) {
      ...
      console.log(`  AcoustID Fingerprint: ${currentTags.acoustidFingerprint || "(none)"}`);
      console.log(`  AcoustID ID: ${currentTags.acoustidId || "(none)"}`);
      console.log(`  MusicBrainz Track ID: ${currentTags.musicbrainzTrackId || "(none)"}`);
      ...
      file.setAcoustidFingerprint("AQADtMmybfGO8NCNEESLnzHyXNOHeHnG4wccz9DR_gGNT_");
      ...
      if (format === "MP3") {
        console.log(`  AcoustID Fingerprint → ${acoustidFingerprintMapping.id3v2?.frame} (${acoustidFingerprintMapping.id3v2?.description})`);
        console.log(`  AcoustID ID → ${acoustidIdMapping.id3v2?.frame} (${acoustidIdMapping.id3v2?.description})`);
        console.log(`  MusicBrainz Track ID → ${mbTrackIdMapping.id3v2?.frame} (${mbTrackIdMapping.id3v2?.description})`);
      } else if (format === "FLAC" || format === "OGG") {
        console.log(`  AcoustID Fingerprint → ${acoustidFingerprintMapping.vorbis}`);
        console.log(`  AcoustID ID → ${acoustidIdMapping.vorbis}`);
        console.log(`  MusicBrainz Track ID → ${mbTrackIdMapping.vorbis}`);
      } else if (format === "MP4") {
        console.log(`  AcoustID Fingerprint → ${acoustidFingerprintMapping.mp4}`);
...
function showMetadataMappingTable() {
  ...
  console.log("├─────────────────┼──────────────────────────────────────┼─────────────────────┼──────────────────────────────────────┤");
  console.log("│ Fingerprint     │ TXXX:Acoustid Fingerprint           │ ACOUSTID_FINGERPRINT│ ----:com.apple.iTunes:Acoustid...   │");
  console.log("│ AcoustID        │ TXXX:Acoustid Id                    │ ACOUSTID_ID         │ ----:com.apple.iTunes:Acoustid Id   │");
  console.log("└─────────────────┴──────────────────────────────────────┴─────────────────────┴──────────────────────────────────────┘");
  ...
  console.log("│ Field           │ MP3 (ID3v2)                         │ FLAC/OGG (Vorbis)  │ MP4/M4A (Atoms)                     │");
  console.log("├─────────────────┼──────────────────────────────────────┼─────────────────────┼──────────────────────────────────────┤");
  console.log("│ Track ID        │ UFID:http://musicbrainz.org         │ MUSICBRAINZ_TRACKID │ ----:com.apple.iTunes:MusicBrainz...│");
  console.log("│ Release ID      │ TXXX:MusicBrainz Album Id           │ MUSICBRAINZ_ALBUMID │ ----:com.apple.iTunes:MusicBrainz...│");
  console.log("│ Artist ID       │ TXXX:MusicBrainz Artist Id          │ MUSICBRAINZ_ARTISTID│ ----:com.apple.iTunes:MusicBrainz...│");
  console.log("└─────────────────┴──────────────────────────────────────┴─────────────────────┴──────────────────────────────────────┘");
  
  console.log("\n💡 Usage Example:");
  console.log("```typescript");
  console.log("// This single call works for ALL formats!");
  console.log('file.setAcoustidFingerprint("AQADtMmybfGO8NCNEESLnzHyXNOHeHnG...");');
  console.log("");
