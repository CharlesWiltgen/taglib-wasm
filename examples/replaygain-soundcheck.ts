...
async function demonstrateReplayGainAndSoundCheck() {
  ...
  try {
    ...
    const appleSoundCheckData = "00000150 00000150 00000150 00000150 00024CA0 00024CA0 00007FFF 00007FFF 00024CA0 00024CA0";
    
    const testFiles = [
      { path: "./test-files/mp3/kiss-snippet.mp3", format: "MP3" },
      { path: "./test-files/flac/kiss-snippet.flac", format: "FLAC" },
      { path: "./test-files/ogg/kiss-snippet.ogg", format: "OGG" },
      { path: "./test-files/mp4/kiss-snippet.m4a", format: "MP4" },
    ];
    ...
    for (const { path, format } of testFiles) {
      ...
      console.log(`  ReplayGain Track Gain: ${currentTags.replayGainTrackGain || "(none)"}`);
      console.log(`  ReplayGain Track Peak: ${currentTags.replayGainTrackPeak || "(none)"}`);
      console.log(`  ReplayGain Album Gain: ${currentTags.replayGainAlbumGain || "(none)"}`);
      console.log(`  ReplayGain Album Peak: ${currentTags.replayGainAlbumPeak || "(none)"}`);
      console.log(`  Apple Sound Check: ${currentTags.appleSoundCheck || "(none)"}`);
      ...
      file.setReplayGainTrackGain(replayGainData.trackGain);
      file.setReplayGainTrackPeak(replayGainData.trackPeak);
      file.setReplayGainAlbumGain(replayGainData.albumGain);
      file.setReplayGainAlbumPeak(replayGainData.albumPeak);
      file.setAppleSoundCheck(appleSoundCheckData);
      ...
      if (format === "MP3") {
        ...
        console.log(`    • ${trackGainMapping.id3v2?.frame} (${trackGainMapping.id3v2?.description})`);
        console.log(`    • ${trackPeakMapping.id3v2?.frame} (${trackPeakMapping.id3v2?.description})`);
        console.log(`    • ${albumGainMapping.id3v2?.frame} (${albumGainMapping.id3v2?.description})`);
        console.log(`    • ${albumPeakMapping.id3v2?.frame} (${albumPeakMapping.id3v2?.description})`);
        console.log(`  Apple Sound Check → ${soundCheckMapping.id3v2?.frame} (${soundCheckMapping.id3v2?.description})`);
    ...
    console.log("• No need to know TXXX frame descriptions or Vorbis field names");
...
function showNormalizationMappingTable() {
  ...
  console.log("\\n🎚️ ReplayGain Fields:");
  console.log("┌─────────────────┬──────────────────────────────────────┬─────────────────────┬──────────────────────────────────────┐");
  console.log("│ Field           │ MP3 (ID3v2)                         │ FLAC/OGG (Vorbis)  │ MP4/M4A (Atoms)                     │");
  console.log("├─────────────────┼──────────────────────────────────────┼─────────────────────┼──────────────────────────────────────┤");
  console.log("│ Track Gain      │ TXXX:ReplayGain_Track_Gain          │ REPLAYGAIN_TRACK_GAIN│ ----:com.apple.iTunes:replaygain... │");
  console.log("│ Track Peak      │ TXXX:ReplayGain_Track_Peak          │ REPLAYGAIN_TRACK_PEAK│ ----:com.apple.iTunes:replaygain... │");
  console.log("│ Album Gain      │ TXXX:ReplayGain_Album_Gain          │ REPLAYGAIN_ALBUM_GAIN│ ----:com.apple.iTunes:replaygain... │");
  console.log("│ Album Peak      │ TXXX:ReplayGain_Album_Peak          │ REPLAYGAIN_ALBUM_PEAK│ ----:com.apple.iTunes:replaygain... │");
  console.log("└─────────────────┴──────────────────────────────────────┴─────────────────────┴──────────────────────────────────────┘");
  ...
  console.log("│ Field           │ MP3 (ID3v2)                         │ FLAC/OGG (Vorbis)  │ MP4/M4A (Atoms)                     │");
  console.log("├─────────────────┼──────────────────────────────────────┼─────────────────────┼──────────────────────────────────────┤");
  console.log("│ Sound Check     │ TXXX:iTunNORM                       │ ITUNNORM            │ ----:com.apple.iTunes:iTunNORM      │");
  console.log("└─────────────────┴──────────────────────────────────────┴─────────────────────┴──────────────────────────────────────┘");
  
  console.log("\\n💡 Usage Examples:");
  console.log("```typescript");
  console.log("// ReplayGain - Single API works for ALL formats!");
  console.log('file.setReplayGainTrackGain("-6.54 dB");');
  console.log('file.setReplayGainTrackPeak("0.987654");');
  console.log("");
  console.log("// Apple Sound Check");
  console.log('file.setAppleSoundCheck("00000150 00000150 00000150...");');
  console.log("");
  console.log("// Bulk setting via extendedTag");
  console.log("file.setExtendedTag({");
  console.log('  replayGainTrackGain: "-6.54 dB",');
  console.log('  replayGainTrackPeak: "0.987654",');
  console.log('  appleSoundCheck: "00000150...",');
  console.log("});");
  console.log("```");
  
  console.log("\\n📖 Technical Notes:");
  console.log("• ReplayGain values: Track/Album gain in dB, peak as decimal 0.0-1.0");
  console.log("• Apple Sound Check: Hex string format used by iTunes/Apple Music"); 
