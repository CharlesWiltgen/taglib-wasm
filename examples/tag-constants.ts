...
import { TagLib, Tags, isValidTagName, getAllTagNames } from "../index.ts";
...
async function demonstrateTagConstants() {
  ...
  try {
    ...
    // Load a test file
    const audioData = await Deno.readFile("./tests/test-files/mp3/kiss-snippet.mp3");
    ...
    console.log(`Track Number: ${properties[Tags.TrackNumber]?.[0] || "(none)"}`);
    ...
    console.log(`Updated Album Artist: ${updatedProperties[Tags.AlbumArtist]?.[0]}`);
    ...
    console.log(`Is "${invalidTag}" a valid tag? ${isValidTagName(invalidTag)}`);
