...
function showPictureTypes() {
  ...
  const types = [
    ...
    { value: PictureType.MovieScreenCapture, name: "Movie/video screen capture" },
...
async function manageCompleteArtwork() {
  ...
  for (const info of metadata) {
    console.log(`  ${PictureType[info.type]}: ${info.mimeType}, ${info.size} bytes`);
  }
}
...
async function smartCoverReplacement() {
  ...
  const newFrontCover = await loadPictureFromFile("better-cover.jpg", PictureType.FrontCover);
...
async function coverArtQualityCheck(audioFile: string) {
  ...
  for (const pic of pictures) {
    ...
    if (sizeKB > 1000) {
      console.log(`  ⚠️  Warning: Large image (${sizeKB} KB) may affect file size`);
    }
    ...
    if (!pic.mimeType.match(/^image\/(jpeg|png)$/)) {
      console.log(`  ⚠️  Warning: Non-standard format (use JPEG/PNG for compatibility)`);
    }
  }
}
