...
const nodeDetectionPattern = /(var ENVIRONMENT_IS_NODE=typeof process=="object"&&process\.versions\?\.node&&process\.type!="renderer")/;
const newNodeDetection = `var ENVIRONMENT_IS_DENO=typeof Deno!=="undefined";$1&&!ENVIRONMENT_IS_DENO`;
...
const nodeModulePattern = /if\(ENVIRONMENT_IS_NODE\)\{const\{createRequire\}=await import\("module"\);var require=createRequire\(import\.meta\.url\)\}/;
const newNodeModule = `if(ENVIRONMENT_IS_NODE&&!ENVIRONMENT_IS_DENO){const{createRequire}=await import("module");var require=createRequire(import.meta.url)}`;
...
const newFileRead = `var readAsync,readBinary;if(ENVIRONMENT_IS_DENO){readBinary=async filename=>{if(filename instanceof URL||filename.startsWith("http")){const resp=await fetch(filename);return new Uint8Array(await resp.arrayBuffer())}else{return await Deno.readFile(filename)}};readAsync=readBinary}else if(ENVIRONMENT_IS_NODE){`;
...
if (matches && matches.length > 0) {
  ...
  modified = true;
  console.log(`  ✓ Fixed WebAssembly instantiation checks (${matches.length} occurrences)`);
}
